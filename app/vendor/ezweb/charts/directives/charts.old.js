define([ //
    'angular', //
    'ezweb/charts/vendor/highcharts/highcharts-all' //
], function (angular) {
    'use strict';
    return angular.module('ezweb.charts.directives', []) //
        .directive('ezCharts', function () {

            // inspired from : https://github.com/pablojim/highcharts-ng
            ...
            function deepExtend(destination, source) {
                for (var property in source) {
                    if (source[property] && source[property].constructor && source[property].constructor
                        === Object) {
            ...
            var getMergedOptions = function (scope, element, config) {
                ...
                axisNames.forEach(function (axisName) {
                    if (config[axisName]) {
                        ...
                        prependMethod(mergedOptions.chart.events, 'addSeries', function (e) {
                            scope.config[axisName].currentMin = this[axisName][0].min
                                || scope.config[axisName].currentMin;
                            scope.config[axisName].currentMax = this[axisName][0].max
                                || scope.config[axisName].currentMax;
                        });
                ...
                if (scope.title) {
                    mergedOptions.title.text = scope.title;
                }
                else if (config.text) {
            ...
            var initialiseChart = function (scope, element, config) {
                ...
                var chart = config.useHighStocks ? new Highcharts.StockChart(mergedOptions)
                ...
                processSeries(chart, scope.data); // scope.data rather than config.series
                if (config.loading) {
                    chart.showLoading();
                }
                chart.redraw();
            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    function initChart() {
                        ...
                        if (typeof(attrs.clickThrough) !== 'undefined') {
